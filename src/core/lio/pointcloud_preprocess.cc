...
void PointCloudPreprocess::RobosenseHandler(const sensor_msgs::msg::PointCloud2::SharedPtr &msg)
{
    ...
    int plsize = pl_orig.points.size();
    if (plsize == 0) {
        LOG(WARNING) << "RoboSense scan is empty";
        return;
    }

    cloud_out_.reserve(plsize);

    const double start_time = pl_orig.points[0].timestamp;
    if (!std::isfinite(start_time)) {
        LOG(WARNING) << "RoboSense scan has an invalid first-point timestamp";
        return;
    }
    scan_start_time_ = start_time;
